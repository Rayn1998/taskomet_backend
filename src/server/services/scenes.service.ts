...
export async function getScenesProgress(sceneId: number): Promise<{ status: number; amount: number }[]> {
	return (
		await dataBasePool.query(
			`
...
export async function getScenesPriority(sceneId: number): Promise<{ priority: number; amount: number }[]> {
	return (
		await dataBasePool.query(
			`
...
export async function createScene(name: string, projectId: number, description: string) {
	const result = await dataBasePool.query(
		`
...
export async function deleteScene(sceneId: number) {
	return await dataBasePool.query(`DELETE FROM scenes WHERE id = $1`, [sceneId]);
}
