...
export const login = async (req: Request, res: Response, next: NextFunction) => {
	...
	if (!receivedPassword || (!email && !userName)) return next(new Error("Enter data for login, please"));
	try {
		const user = email ? await getArtist({ email }) : await getArtist({ user_name: userName });
		const passVerification = await verifyPassword(receivedPassword, user!.password);
		if (!user || !passVerification) return res.status(401).json({ message: "Invalid credentials" });
...
export const sendUserData = async (req: Request, res: Response, next: NextFunction) => {
	if (!(req.headers.cookie && req.headers.cookie.startsWith("session")))
		return res.status(401).json({ message: "Unauthorized" });
	const cookie = getCookie(req, "session");
	const session = await sessionController.checkSession(cookie!);
