...
export async function getProjects(
  ...
  try {
    ...
    for (const project of projects) {
      const progress = await projectService.getProjectsProgress(project.id);
      const priority = await projectService.getProjectsPriority(project.id);
...
export async function deleteProject(
  ...
  if (!projectId) return next(new Error("Project id not provided: projectId"));

  try {
    await dataBasePool.query("BEGIN");

    const sceneIds = (
      await dataBasePool.query(`SELECT * FROM scenes WHERE project = $1;`, [
        projectId,
      ])
