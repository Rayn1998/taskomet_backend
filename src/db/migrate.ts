...
class Migrate {
	...
	async connectToDb() {
		try {
			...
			if (isDatabaseError(err)) {
				if (err.code === "28P01") {
					console.log(chalk.red("Incorrect password to db"));
				}
				process.exit(1);
			} else {
				console.error(err);
	...
	async recreateDb() {
		...
		try {
			...
			console.log(chalk.green("Database recreated successfully"));
		} catch (err) {
			console.error(err);
		} finally {
			sysClient.release();
	...
	async createTables() {
		try {
			...
			console.log(chalk.green("Tables succsessfully created"));
		} catch (err) {
			console.error(err);
		}
	}
	...
	async addArtists() {
		try {
            ...
            VALUES 
                ...
                ('Test User 1', 'bodolanov1', 'test1@mail.ru', '4fd4ddb5f003396885fc4db4d572fa5c:423302ceb85216cce76d0bb069a779da94d5510ece77893496163227ab4b2342afae5168e72943a28443721700c7b461db996d08e583966d609e497db1cd8de5', 0),
                ...
                ('Test User 3', 'bodolanov3', 'test3@mail.ru', '4fd4ddb5f003396885fc4db4d572fa5c:423302ceb85216cce76d0bb069a779da94d5510ece77893496163227ab4b2342afae5168e72943a28443721700c7b461db996d08e583966d609e497db1cd8de5', 2);
            `);
		} catch (err) {
			console.error(err);
		}
...
async function removeUploads() {
	try {
		...
		const exist = await fs.access(uploadsPath);
		if (typeof exist === "undefined") return await fs.rm(uploadsPath, { recursive: true });
	} catch (err) {
		console.log(chalk.green("'uploads' folder doesn't exist, nothing to delete"));
	}
}
...
async function migration() {
	...
	try {
		...
		try {
			...
			console.error(chalk.red("Migration failed, transaction rolled back", err));
		}
	} catch (err) {
		console.error(chalk.red("Migration script failed", err));
		process.exit(1);
	} finally {
